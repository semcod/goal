...
def _detect_package_manager(project_root: Path, pm: PackageManager) -> bool:
    return _has_any_matching_path(
        project_root, pm.lock_files
    ) or _has_any_matching_path(project_root, pm.config_files)
...
def detect_package_managers(project_path: str = ".") -> List[PackageManager]:
    ...
    detected = [
        ...
        if _detect_package_manager(project_root, pm)
    ]
...
def get_preferred_package_manager(
    project_path: str = ".", language: Optional[str] = None
...
def format_package_manager_command(
    ...
    try:
        ...
        raise ValueError(
            f"Missing required parameter {e} for command '{command_type}' in {pm.name}"
        )
