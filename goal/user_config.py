...
class UserConfig:
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            click.echo(
                click.style(f"Warning: Could not save user config: {e}", fg="yellow")
            )
    ...
    def is_initialized(self) -> bool:
        ...
        return (
            "author_name" in self.config
            and "author_email" in self.config
            and "license" in self.config
        )
...
def prompt_for_license() -> Dict[str, str]:
    ...
    for num, info in sorted(AVAILABLE_LICENSES.items()):
        click.echo(
            f"  {click.style(num, fg='green', bold=True)}. {click.style(info['name'], fg='white', bold=True)}"
        )
    ...
    while True:
        choice = click.prompt(
            click.style("Enter your choice", fg="cyan"), type=str, default="1"
        ).strip()
        ...
        if choice in AVAILABLE_LICENSES:
            ...
            click.echo(
                click.style(
                    f"Invalid choice '{choice}'. Please enter a number from 1-{len(AVAILABLE_LICENSES)}.",
                    fg="red",
                )
            )
...
def initialize_user_config(force: bool = False) -> UserConfig:
    ...
    if git_name:
        click.echo(
            f"✓ Detected git user.name: {click.style(git_name, fg='green', bold=True)}"
        )
    if git_email:
        click.echo(
            f"✓ Detected git user.email: {click.style(git_email, fg='green', bold=True)}"
        )
    ...
    if not git_name or not git_email:
        ...
        click.echo(
            click.style(
                "⚠ Git user information not configured!", fg="yellow", bold=True
            )
        )
    ...
    click.echo(
        f"  Author: {click.style(git_name, fg='white', bold=True)} <{click.style(git_email, fg='white', bold=True)}>"
    )
    click.echo(f"  License: {click.style(license_info['name'], fg='white', bold=True)}")
    click.echo()
    click.echo(
        click.style(
            "💡 Tip: Run 'goal config' to view or change these settings anytime",
            fg="cyan",
        )
    )
    click.echo()
...
def show_user_config() -> None:
    ...
    if not config.is_initialized():
        click.echo(
            click.style(
                "No user configuration found. Run any goal command to initialize.",
                fg="yellow",
            )
        )
        return
    ...
    click.echo()
    click.echo(
        f"Config file: {click.style(str(config.config_path), fg='white', bold=True)}"
    )
    click.echo()
    click.echo("Current settings:")
    click.echo(
        f"  Author name:  {click.style(config.get('author_name', 'Not set'), fg='green', bold=True)}"
    )
    click.echo(
        f"  Author email: {click.style(config.get('author_email', 'Not set'), fg='green', bold=True)}"
    )
    click.echo(
        f"  License:      {click.style(config.get('license_name', 'Not set'), fg='green', bold=True)} ({config.get('license', 'Not set')})"
    )
    click.echo()
    click.echo(
        click.style("💡 Tip: Run 'goal config --reset' to reconfigure", fg="cyan")
    )
    click.echo()
