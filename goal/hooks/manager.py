    ...
    def is_precommit_installed(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["pre-commit", "--version"], capture_output=True, check=True
            )
            return True
...
'''
    ...
    def create_precommit_config(self, force: bool = False) -> bool:
        ...
        if self.precommit_file.exists() and not force:
            click.echo(
                click.style("⚠ .pre-commit-config.yaml already exists", fg="yellow")
            )
    ...
    def install_hooks(self, force: bool = False) -> bool:
        ...
        click.echo(click.style("✓ Created hook script", fg="green"))
    ...
    def uninstall_hooks(self) -> bool:
        ...
        try:
            subprocess.run(
                ["pre-commit", "uninstall"], check=True, cwd=self.project_dir
            )
        except subprocess.CalledProcessError:
        ...
        if self.goal_hook_script.exists():
            ...
            click.echo(click.style("✓ Removed hook script", fg="green"))
