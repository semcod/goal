...
def _handle_push_failure(result, branch: str, yes: bool) -> bool:
    ...
    if not yes and result.stderr:
        ...
        click.echo(
            click.style(
                "Push failed. Run 'goal recover' to attempt automatic recovery.",
                fg="yellow",
            )
        )
        sys.exit(1)
...
def push_to_remote(
    ...
    if not has_remote:
        click.echo(
            click.style(
                "  ℹ  No remote configured — commit saved locally.", fg="yellow"
            )
        )
        return False
...
def _handle_large_files_in_history(error_output: str) -> bool:
    ...
    click.echo(
        click.style(
            "\n⚠️  These files are already committed in git history!",
            fg="red",
            bold=True,
        )
    )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if len(ahead_behind) == 2:
                ...
                if ahead > 0:
                    click.echo(
                        f"    • {ahead} commit(s) ahead of remote (local changes to push)"
                    )
                if behind > 0:
                    click.echo(
                        f"    • {behind} commit(s) behind remote (remote changes not in local)"
                    )
                if ahead == 0 and behind == 0:
                    click.echo("    • Same commit count, but histories diverged")
...
def _handle_force_push() -> bool:
    ...
    if click.confirm(click.style("Are you sure?", fg="red", bold=True)):
        try:
            ...
            if result.returncode == 0:
                click.echo(
                    click.style("✅ Force push successful!", fg="green", bold=True)
                )
                return True
            else:
                click.echo(
                    click.style(f"❌ Force push failed: {result.stderr}", fg="red")
                )
                return False
        except Exception as e:
...
def _handle_pull_merge() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            click.echo(
                click.style("✅ Pull successful! Remote changes merged.", fg="green")
            )
            click.echo(
                click.style(
