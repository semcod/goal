...
def handle_todo_stage(ctx_obj: dict, yes: bool, dry_run: bool) -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            details = (result.stderr or result.stdout or "prefact exited with non-zero status").strip()
            click.echo(click.style(f"  ❌ prefact failed: {details}", fg='red'))
            click.echo(click.style("  Tip: run `prefact -a` manually to inspect/fix the issue.", fg='yellow'))
            return False
        ...
        if not existing_candidates:
            click.echo(click.style("  ℹ️ prefact ran, but no TODO/planfile artifacts were found to stage.", fg='yellow'))
            return True
        ...
        if failed_paths:
            ...
            click.echo(click.style(f"  ❌ Failed to stage prefact artifacts: {failed_list}", fg='red'))
            click.echo(click.style("  Tip: inspect git status and stage files manually.", fg='yellow'))
            return False
        ...
        click.echo(click.style("  Tip: install `prefact` or run push without `--todo`.", fg='yellow'))
        return False
