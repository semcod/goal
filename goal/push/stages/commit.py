...
def get_commit_message(
    ...
    if abstraction != "legacy" and config_dict:
        abstraction_result = generator.generate_abstraction_message(
            level=abstraction, cached=True
        )
...
def _build_validation_summary(
    commit_msg: str, detailed_result: Dict, total_adds: int, total_dels: int
) -> Dict[str, Any]:
    return {
        "title": commit_msg,
...
def _confirm_suggested_title(commit_msg: str, suggested_title: str, yes: bool) -> bool:
    ...
    if yes:
        return True
    return confirm(
        f"Apply suggested title?\n\nCurrent: {commit_msg}\nSuggested: {suggested_title}"
    )


def _echo_applied_title_fix(
    ctx_obj: Dict[str, Any], commit_msg: str, markdown: bool
) -> None:
...
def enforce_quality_gates(
    ...
    try:
        ...
        summary = _build_validation_summary(
            commit_msg, detailed_result, total_adds, total_dels
        )
...
def _commit_file_group(
    ...
    if result.returncode != 0:
        click.echo(
            click.style(
                f"✗ Error committing split group {gname}: {result.stderr}", fg="red"
            )
        )
        if not yes:
            sys.exit(1)
...
def _commit_release_metadata(
    ...
    release_title = apply_ticket_prefix(
        f"chore(release): bump version to {new_version}", ticket
    )
...
def handle_split_commits(
    ...
    if not yes:
        ...
        for gname in _GROUP_ORDER:
            if gname in groups:
                d = generator.generate_detailed_message(
                    cached=False, paths=groups[gname]
                )
