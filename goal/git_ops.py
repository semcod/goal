...
def _prompt_remote_url() -> Optional[str]:
    ...
    click.echo(
        click.style("  SSH   example: git@github.com:user/repo.git", fg="bright_black")
    )
    click.echo(
        click.style(
            "  HTTP  example: https://github.com/user/repo.git", fg="bright_black"
        )
    )
...
def clone_repository(url: str, target_dir: Optional[str] = None) -> Tuple[bool, str]:
    ...
    if result.returncode == 0:
        ...
        return (
            False,
            "Failed to clone repository. Check the URL and your access permissions.",
        )
...
def _handle_merge_strategy(branches: list, has_files: bool) -> None:
    ...
    click.echo(
        click.style(f"  Remote branches: {', '.join(branches)}", fg="bright_black")
    )
    click.echo()
    ...
    if merge_action == 2:
        ...
        result = _run_git_verbose(
            "merge", f"origin/{branch}", "--allow-unrelated-histories", "--no-edit"
        )
        if result.returncode != 0:
            click.echo(
                click.style(
                    "⚠  Merge conflict detected. Resolve manually, then run goal again.",
                    fg="yellow",
                    bold=True,
                )
            )
            click.echo(
                click.style(
                    "  Hint: git status  →  resolve  →  git add .  →  git merge --continue",
                    fg="bright_black",
                )
            )
        else:
            click.echo(
                click.style(f"✓ Merged origin/{branch} into local files.", fg="green")
            )
    elif merge_action == 3:
        result = _run_git_verbose("checkout", f"origin/{branch}", "-B", branch)
        if result.returncode == 0:
            click.echo(
                click.style(f"✓ Local files replaced with origin/{branch}.", fg="green")
            )
        else:
            click.echo(
                click.style(f"✗ Failed to checkout: {result.stderr.strip()}", fg="red")
            )
...
def _handle_init_remote(has_files: bool) -> bool:
    ...
    if url:
        ...
        if result.returncode != 0:
            click.echo(
                click.style(
                    f"✗ Could not add remote: {result.stderr.strip()}", fg="red"
                )
            )
        else:
        ...
        if fetch_result.returncode == 0:
            ...
            click.echo(
                click.style(
                    f"⚠  Could not fetch remote: {fetch_result.stderr.strip()}",
                    fg="yellow",
                )
            )
            click.echo(
                click.style(
                    "  Remote is configured but unreachable. Check URL and credentials.",
                    fg="bright_black",
                )
            )
...
def _handle_local_init() -> bool:
    ...
    if result.returncode == 0:
        click.echo(
            click.style(
                "✓ Initialized local git repository (no remote).", fg="green", bold=True
            )
        )
        return True
    click.echo(click.style("✗ Failed to initialize git repository.", fg="red"))
...
def ensure_git_repository(auto: bool = False) -> bool:
    ...
    if auto:
        click.echo(
            click.style("  Skipping (--all mode, no interactive prompts).", fg="yellow")
        )
        return False
...
def ensure_remote(auto: bool = False) -> bool:
    ...
    if auto:
        click.echo(
            click.style(
                "  Skipping remote setup (--all mode). Commit will be local only.",
                fg="yellow",
            )
        )
        return False
    ...
    if action == 1:
        ...
        if verify.returncode == 0:
            ...
            click.echo(
                click.style(
                    "  ⚠  Remote is not reachable (check URL/credentials). Push may fail.",
                    fg="yellow",
                )
            )
        return True
