...
class CommitMessageGenerator:
    ...
    def classify_change_type(
        self, files: List[str], diff_content: str, stats: Dict[str, int]
    ) -> str:
        """Classify the type of change using pattern matching and heuristics."""
        if self._change_analyzer:
            return self._change_analyzer.classify_change_type(
                files, diff_content, stats
            )
        return "chore"
    ...
    def generate_abstraction_message(
        self, level: str = "auto", cached: bool = True
        ...
        try:
            ...
            actual_level = (
                level
                if level != "auto"
                else self._smart_generator.abstraction.determine_abstraction_level(
                    analysis
                )
            )
    ...
    def generate_changelog_entry(
        self, cached: bool = True, commit_hash: str = None
    ) -> Optional[Dict[str, Any]]:
    ...
    def _try_enhanced_summary(
        self, cached: bool, paths: Optional[List[str]]
    ...
    def _classify_files(
        self, name_status: List[Tuple[str, str]], files: List[str]
        ...
        modified_files = [
            ...
            if s.startswith("M") or s.startswith("R") or s.startswith("C")
        ]
    ...
    def _build_summary_section(
        ...
        parts = ["\nSummary:"]
        parts.append(
            f"- Dirs: {', '.join(f'{k}={v}' for k, v in dir_counter.most_common(6))}"
        )
        parts.append(
            f"- Exts: {', '.join(f'{k}={v}' for k, v in ext_counter.most_common(6))}"
        )
        parts.append(
            f"- A/M/D: {len(added_files)}/{len(modified_files)}/{len(deleted_files)}"
        )
        if added_files:
            parts.append(
                f"- Added: {', '.join(added_files[:8])}{' ...' if len(added_files) > 8 else ''}"
            )
        if deleted_files:
            parts.append(
                f"- Deleted: {', '.join(deleted_files[:8])}{' ...' if len(deleted_files) > 8 else ''}"
            )
        if symbols:
    ...
    def _build_per_file_notes(
        self, files: List[str], numstat_map: Dict[str, Tuple[int, int]], cached: bool
    ) -> List[str]:
    ...
    def generate_detailed_message(
        ...
        added_files, deleted_files, modified_files = self._classify_files(
            name_status, files
        )
