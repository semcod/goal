...
class CodeAbstraction:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.config = config
        self.domain_mapping = (
            config.get("git", {}).get("commit", {}).get("domain_mapping", {})
        )
        self.code_parsers = config.get("code_parsers", {})
        self.benefit_keywords = (
            config.get("git", {}).get("commit", {}).get("benefit_keywords", {})
        )
        self.templates = config.get("git", {}).get("commit", {}).get("templates", {})
        self.abstraction_level = (
            config.get("git", {}).get("commit", {}).get("abstraction_level", "auto")
        )
    ...
    def get_domain(self, filepath: str) -> str:
        ...
        for pattern, domain in self.domain_mapping.items():
            ...
            if (
                pattern.endswith("/*.py")
                and filepath.startswith(pattern[:-4])
            ...
            if line.startswith("+") and not line.startswith("+++"):
                ...
                if line.startswith("#"):
                    ...
                    if topic and len(topic) > 2:
                        # Skip noise patterns
                        if not any(
                            re.match(p, topic, re.IGNORECASE) for p in ignore_patterns
                        ):
                            topics.append(topic)
