...
class PostCommitManager:
    ...
    def run_actions(self) -> bool:
        ...
        click.echo()
        click.echo(
            click.style("🚀 Running post-commit actions...", fg="cyan", bold=True)
        )
        click.echo("-" * 40)
        ...
        for action_config in actions_config:
            ...
            if action_type not in AVAILABLE_ACTIONS:
                click.echo(
                    click.style(f"⚠ Unknown action type: {action_type}", fg="yellow")
                )
                continue
            ...
            if not action.validate_config():
                click.echo(
                    click.style(f"✗ Invalid configuration for {action_type}", fg="red")
                )
                all_succeeded = False
                continue
    ...
    def validate_actions(self) -> bool:
        ...
        for action_config in actions:
            ...
            if action_type not in AVAILABLE_ACTIONS:
                click.echo(
                    click.style(f"✗ Unknown action type: {action_type}", fg="red")
                )
                all_valid = False
                continue
            ...
            if not action.validate_config():
                ...
                click.echo(
                    click.style(f"✓ {action_type} configuration valid", fg="green")
                )
