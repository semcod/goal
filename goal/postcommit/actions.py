...
class NotificationAction(PostCommitAction):
    ...
    def execute(self, commit_info: Dict[str, str]) -> bool:
        ...
        message = self.config.get(
            "message", f"Committed: {commit_info.get('message', '')[:50]}..."
        )
...
class WebhookAction(PostCommitAction):
    ...
    def execute(self, commit_info: Dict[str, str]) -> bool:
        try:
            ...
            click.echo(
                click.style(
                    "⚠ requests library not installed, webhook skipped", fg="yellow"
                )
            )
            return False
    ...
    def execute(self, commit_info: Dict[str, str]) -> bool:
        ...
        if not branch:
            ...
            try:
                ...
                click.echo(
                    click.style("✗ Could not determine current branch", fg="red")
                )
                return False

        try:
            subprocess.run(
                ["git", "push", remote, branch], check=True, capture_output=True
            )
