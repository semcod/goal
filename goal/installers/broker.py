...
class PackageManagerBroker:
    ...
    def detect_available(self) -> list[AbstractPackageManager]:
        ...
        if self._available is None:
            self._available = sorted(
                [m for m in _MANAGERS if m.is_available()], key=lambda m: m.priority
            )
    ...
    def install(
        ...
        if use_lockfile:
            ...
            if lockfile_result:
                ...
                click.echo("⚠️  Lockfile install failed, trying editable install...")
    ...
    def _select_manager(
        self, available: list[AbstractPackageManager], prefer: Optional[str]
    ...
    def _report(
        self, result: InstallResult, available: list[AbstractPackageManager]
    ) -> None:
    ...
    def install_smart(
        ...
        if lockfile:
            ...
            return self.install(
                extras=extras, prefer=prefer, auto_install_uv=auto_install_uv
            )
    ...
    def show_available(self) -> None:
        ...
        for m in available:
            ...
            lock_match = (
                "🔒" if lockfile and _LOCKFILE_MANAGERS.get(lockfile) == m.name else ""
            )
