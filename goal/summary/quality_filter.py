...
class SummaryQualityFilter:
    ...
    FEAT_PATTERNS = [r"new_", r"initial", r"support", r"implement", r"create"]
    FIX_PATTERNS = [r"fix", r"bug", r"issue", r"error", r"patch", r"hotfix"]
    ...
    def __init__(self):
        ...
        self._refactor_re = [
            re.compile(p, re.IGNORECASE) for p in self.REFACTOR_PATTERNS
        ]
    ...
    def filter_entities(self, entities: List[Dict]) -> List[Dict]:
        ...
        return [
            e
            for e in entities
            if not self.is_noise(e.get("name", ""), e.get("role", ""))
        ]
    ...
    def format_complexity_delta(
        self, old_complexity: int, new_complexity: int
    ) -> Tuple[str, str]:
    ...
    def _resolve_scored_intent(scores: Dict[str, int], net: int, deleted: int) -> str:
        ...
        if (
            deleted >= 100
            and scores.get("refactor", 0) >= scores.get("feat", 0)
            and scores.get("refactor", 0) >= scores.get("fix", 0)
        ):
            return "refactor"
        return max(scores, key=scores.get)

    def classify_intent_smart(
        self, files: List[str], entities: List[Dict], added: int = 0, deleted: int = 0
    ) -> str:
