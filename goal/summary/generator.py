...
class EnhancedSummaryGenerator:
    ...
    _SPECIAL_TITLE_RULES = [
        ...
        (
            lambda s: "analyzer" in s or "analysis" in s,
            None,
            lambda s: (
                "intelligent code analysis pipeline"
                if any("deep" in x or "smart" in x for x in s)
                else "code analysis engine"
            ),
        ),
        # Commit-related files
        (
            lambda s: "commit" in s,
            None,
            lambda s: (
                "smart commit generation system"
                if any("smart" in x for x in s)
                else "commit message generator"
            ),
        ),
    ...
    def _special_title_from_files(file_stems: List[str], areas: List[str]) -> str:
        ...
        for (
            ...
            area_key,
            title_fn,
        ) in EnhancedSummaryGenerator._SPECIAL_TITLE_RULES:
            ...
            for imp in imports:
                ...
                for other_f in files:
                    ...
                    if imp_name == other_name and f != other_f:
                        relations.append(
                            {"from": Path(f).stem, "to": other_name, "type": "imports"}
                        )
    ...
    def calculate_quality_metrics(
        self, analysis: Dict[str, Any], files: List[str]
    ) -> Dict[str, Any]:
        ...
        total_entities = len(aggregated.get("added_entities", [])) + len(
            aggregated.get("modified_entities", [])
        )
        if total_entities > 0:
            metrics["functional_coverage"] = min(
                100, int((areas / max(total_entities, 1)) * 100 + 50)
            )
        ...
        test_entities = [
            e
            for e in aggregated.get("added_entities", [])
    ...
    def generate_enhanced_summary(
        ...
        relations["relations"] = self.quality_filter.dedupe_relations(
            relations.get("relations", [])
        )
        # Remove generic nodes, then rebuild chain and ASCII after cleaning
        relations["relations"] = self.quality_filter.filter_generic_nodes(
            relations["relations"]
        )
        ...
        for entity in aggregated.get("added_entities", [])[:10]:
            ...
            if role != name:  # Only if mapped to meaningful role
                roles.append(
                    {"name": name, "role": role, "type": entity.get("type", "function")}
                )
    ...
    def validate_summary_quality(self, title: str, body: str) -> Dict[str, Any]:
        ...
        if metric_count < required_metrics:
            warnings.append(
                f"Only {metric_count} metrics found (need {required_metrics})"
            )
