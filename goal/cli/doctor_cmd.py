...
@click.option(
    ...
    "-m",
    default=None,
    help="Force specific package manager (uv, pip, poetry, pdm)",
)
@click.pass_context
    ...
    if detected:
        ...
        for ptype, dirs in detected.items():
            for project_dir in dirs:
                report = diagnose_and_report_with_todo(
                    project_dir,
                    ptype,
                    auto_fix=fix,
                    todo_file=str(todo_file) if todo else None,
                )
        ...
        if total_issues > 0 or total_fixed > 0:
            ...
            if total_issues == 0:
                click.echo(
                    click.style("✓ All projects are healthy!", fg="green", bold=True)
                )
            else:
                click.echo(
                    click.style(
                        f"🩺 Doctor summary: {total_issues} issue(s) found",
                        fg="cyan",
                        bold=True,
                    )
                )
        ...
        click.echo(
            click.style("No known project types detected for diagnostics.", fg="yellow")
        )
