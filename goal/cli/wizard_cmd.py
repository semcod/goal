...
def wizard(reset, skip_git, skip_user, skip_project) -> None:
    ...
    click.echo()
    click.echo(
        click.style("🧙‍♂️  Welcome to the Goal Setup Wizard!", fg="cyan", bold=True)
    )
    ...
    click.echo(
        click.style(
            "✨ Setup complete! You're ready to use Goal.", fg="green", bold=True
        )
    )
    click.echo()
...
def _setup_git_repository():
    ...
    if git_root:
        click.echo(
            f"✓ Already in a git repository: {click.style(str(git_root), fg='green')}"
        )
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if click.confirm(
                    click.style("Add a remote now?", fg="cyan"), default=False
                ):
                    _add_git_remote()
        ...
        if click.confirm(
            click.style("Initialize a git repository?", fg="cyan"), default=True
        ):
            try:
                ...
                click.echo(
                    click.style(f"✗ Failed to initialize git repository: {e}", fg="red")
                )
                return

            # Optionally add remote
            if click.confirm(
                click.style("Add a remote repository?", fg="cyan"), default=False
            ):
                _add_git_remote()
...
def _add_git_remote():
    """Add a git remote."""
    remote_name = click.prompt(click.style("Remote name", fg="cyan"), default="origin")

    remote_url = click.prompt(click.style("Remote URL", fg="cyan"), type=str)
    ...
    try:
        ...
        if result.returncode == 0:
            click.echo(
                click.style(f"✓ Added remote {remote_name} -> {remote_url}", fg="green")
            )
        else:
            click.echo(
                click.style(
                    f"✗ Failed to add remote: {result.stderr.decode()}", fg="red"
                )
            )
    except Exception as e:
...
def _setup_user_config(reset: bool):
    ...
    if config.is_initialized() and not reset:
        ...
        click.echo(
            f"  License: {click.style(config.get('license_name', 'Not set'), fg='green')}"
        )

        if not click.confirm(
            click.style("Update user configuration?", fg="cyan"), default=False
        ):
            return
    ...
    if not git_name or reset:
        git_name = click.prompt(
            click.style("Author name", fg="cyan"), default=git_name or "Unknown Author"
        )
...
def _setup_project_config():
    ...
    if config.exists():
        ...
        if not click.confirm(
            click.style("Update project configuration?", fg="cyan"), default=False
        ):
            return
    ...
    if click.confirm(
        click.style("Customize project settings?", fg="cyan"), default=False
    ):
        # Project name
        name = click.prompt(
        ...
        auto_changelog = click.confirm(
            click.style("Automatically update CHANGELOG.md?", fg="cyan"), default=True
        )
...
def _show_setup_summary():
    ...
    if user_config.is_initialized():
        ...
        click.echo(
            f"  Author: {user_config.get('author_name')} <{user_config.get('author_email')}>"
        )
    ...
    if git_root:
        ...
        try:
            ...
            if result.stdout.strip():
                click.echo(
                    click.style("  Status: Has uncommitted changes", fg="yellow")
                )
            else:
