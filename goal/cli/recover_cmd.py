...
@click.option("--error-message", "-m", help="Provide error message directly")
@click.option(
    "--no-backup", is_flag=True, help="Skip creating backup branch (not recommended)"
)
@click.option("--verbose", "-v", is_flag=True, help="Show detailed recovery steps")
@click.pass_context
def recover(ctx, full, error_file, error_message, no_backup, verbose) -> None:
    ...
    if not error_output:
        click.echo(
            click.style(
                "No error output found. Run 'git push' to see the error.", fg="yellow"
            )
        )
        return
...
def _get_error_output(
    error_file: Optional[str], error_message: Optional[str]
) -> Optional[str]:
