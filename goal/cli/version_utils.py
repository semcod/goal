...
def update_version_in_file(
    filepath: Path, pattern: str, old_version: str, new_version: str
) -> bool:
...
def _build_author_block(
    existing_authors: str, author_name: str, author_email: str
) -> str:
    author_entries = []
...
def _update_tomlkit_authors(
    doc: TOMLDocument, author_name: str, author_email: str
) -> None:
    """Update authors in tomlkit document."""
    if "project" in doc and "authors" in doc["project"]:
        authors = doc["project"]["authors"]
        author_exists = any(
            isinstance(a, dict) and a.get("email") == author_email for a in authors
        )
...
def _update_pyproject_with_tomlkit(
    content: str,
    author_name: str,
    author_email: str,
    license_id: str,
...
def _update_regex_authors(content: str, author_name: str, author_email: str) -> str:
    ...
    if authors_match and author_email not in authors_match.group(1):
        authors_block = _build_author_block(
            authors_match.group(1), author_name, author_email
        )
        content = re.sub(
            r"authors\s*=\s*\[.*?\]", authors_block, content, flags=re.DOTALL
        )
    return content
...
def _update_pyproject_with_regex(
    content: str,
    author_name: str,
    author_email: str,
    license_id: str,
...
def _update_pyproject_metadata(
    ...
    # Try tomlkit first
    result = _update_pyproject_with_tomlkit(
        content, author_name, author_email, license_id, license_classifier
    )
    if result is not None:
        return result

    # Fallback to regex-based approach
    return _update_pyproject_with_regex(
        content, author_name, author_email, license_id, license_classifier
    )


def _update_package_json_metadata(
    content: str, author_name: str, author_email: str, license_id: str
) -> str:
    ...
    if "author" in data:
        if isinstance(data["author"], dict):
            ...
            data["author"] = {"name": author_name, "email": author_email}
    else:
        data["author"] = {"name": author_name, "email": author_email}
...
def update_project_metadata(filepath: Path, user_config) -> bool:
    ...
    try:
        ...
        if filepath.name == "pyproject.toml":
            content = _update_pyproject_metadata(
                content, author_name, author_email, license_id, license_classifier
            )
        elif filepath.name == "package.json":
            content = _update_package_json_metadata(
                content, author_name, author_email, license_id
            )
        elif filepath.name == "setup.py":
            content = _update_setup_py_metadata(
                content, author_name, author_email, license_id
            )
