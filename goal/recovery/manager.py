...
class RecoveryManager:
        ...
        try:
            ...
            if success:
                ...
                if push_result:
                    click.echo(
                        click.style(
                            "\n✅ Recovery successful! Push completed.",
                            fg="green",
                            bold=True,
                        )
                    )
    ...
    def _create_backup(self) -> str:
        ...
        try:
            ...
            click.echo(
                click.style(f"⚠️ Could not create backup branch: {e}", fg="yellow")
            )
            return None
    ...
    def _attempt_push(self) -> bool:
        ...
        try:
            result = self.run_git(
                "push", "origin", "HEAD", capture_output=True, check=False
            )
    ...
    def identify_new_commits(self) -> List[str]:
        ...
        try:
            ...
            result = self.run_git(
                "log", "--oneline", "origin/main..HEAD", capture_output=True, check=True
            )
    ...
    def cherry_pick_commits(self, commits: List[str]) -> bool:
        ...
        try:
            ...
            for commit in reversed(commits):  # Apply in correct order
                try:
                    subprocess.run(
                        ["git", "cherry-pick", commit], check=True, capture_output=True
                    )
                    click.echo(f"✓ Cherry-picked {commit}")
                except subprocess.CalledProcessError as e:
                    click.echo(
                        click.style(f"✗ Failed to cherry-pick {commit}: {e}", fg="red")
                    )
                    # Abort cherry-pick
