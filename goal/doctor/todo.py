...
def add_issues_to_todo(
    project_dir: Path, issues: List[Issue], todo_file: str = "TODO.md"
) -> int:
    ...
    for issue in issues:
        ...
        if ticket_id not in existing_tickets:
            ...
            existing_tickets.add(
                ticket_id
            )  # Mark as added to avoid duplicates in this run
            added_count += 1
    ...
    click.echo(
        click.style(f"  📝 Added {added_count} new issue(s) to {todo_file}", fg="cyan")
    )
    return added_count


def diagnose_and_report_with_todo(
    project_dir: Path,
    project_type: str,
    auto_fix: bool = True,
