...
class PythonDiagnostics(PythonDiagnosticsCore):
    ...
    def _collect_py010_inconsistencies(
        self, pyproject_name: str, setup_py: Path, goal_yaml: Path
    ) -> List[str]:
        ...
        if goal_yaml.exists():
            ...
            goal_name_match = re.search(
                r"^\s*name:\s*(\S+)", goal_content, re.MULTILINE
            )
    ...
    def check_py010_project_name_consistency(self) -> None:
        ...
        inconsistencies = self._collect_py010_inconsistencies(
            pyproject_name, setup_py, goal_yaml
        )
        ...
        issue = Issue(
            severity="error",
            code="PY010",
            title="Inconsistent project name",
            detail=detail,
            file="pyproject.toml",
        )
    ...
    def check_py011_version_consistency(self) -> None:
        ...
        version_match = re.search(
            r'^version\s*=\s*"([^"]+)"', self.content, re.MULTILINE
        )
    ...
    def check_py012_dist_cleanup(self) -> None:
        ...
        detail = (
            f"Found {len(stale_files)} stale file(s) in dist/: {', '.join(stale_files)}"
        )
        issue = Issue(
            severity="error",
            code="PY012",
            title="Stale files in dist/",
            detail=detail,
            file="dist/",
        )
    ...
    def check_py013_goal_publish_pattern(self) -> None:
        ...
        detail = (
            f"Incorrect publish pattern: '{publish_pattern}'. Expected: '{expected}'"
        )
        issue = Issue(
            severity="error",
            code="PY013",
            title="Wrong publish pattern",
            detail=detail,
            file="goal.yaml",
        )
    ...
    def _is_publish_enabled(goal_content: str) -> bool:
        return (
            "publish_enabled: true" in goal_content
            or "publish_enabled:true" in goal_content
        )
