...
class PythonDiagnosticsCore:
    ...
    def check_py001_missing_config(
        self, pyproject: Path, setup_py: Path, setup_cfg: Path, requirements: Path
    ) -> None:
    ...
    def check_py002_build_system(self) -> None:
        ...
        if self.auto_fix:
            ...
            issue.fix_description = (
                "Added [build-system] section with setuptools backend"
            )
    ...
    def check_py003_license_classifiers(self) -> None:
        ...
        detail = (
            ...
            "Found: "
            + ", ".join(c.strip().strip('"').strip(",") for c in license_classifiers)
        )
        issue = Issue(
            severity="error",
            code="PY003",
        ...
        if self.auto_fix:
            ...
            issue.fixed = True
            issue.fix_description = (
                "Removed deprecated License :: classifier(s) from classifiers list"
            )
        self.issues.append(issue)
    ...
    def check_py004_deprecated_backends(self) -> None:
        ...
        for backend in DEPRECATED_BACKENDS:
            ...
            if self.auto_fix:
                ...
                issue.fixed = True
                issue.fix_description = (
                    f'Replaced "{backend}" with "setuptools.build_meta"'
                )
            self.issues.append(issue)
    ...
    def check_py005_license_table(self) -> None:
        ...
        license_string = re.search(
            r'^license\s*=\s*"([^"]+)"', self.content, re.MULTILINE
        )
        ...
        if self.auto_fix:
            ...
            issue.fixed = True
            issue.fix_description = (
                f'Converted license to string expression: license = "{license_id}"'
            )
        self.issues.append(issue)
    ...
    def check_py006_duplicate_authors(self) -> None:
        ...
        author_lines = [
            l.strip().rstrip(",")
            for l in authors_block.strip().splitlines()
        ...
        if self.auto_fix:
            ...
            new_block += "]"
            self.content = re.sub(
                r"authors\s*=\s*\[.*?\]", new_block, self.content, flags=re.DOTALL
            )
            issue.fixed = True
            issue.fix_description = (
                f"Removed duplicate authors (kept {len(unique)} unique)"
            )
        self.issues.append(issue)
    ...
    def check_py009_string_authors(self) -> None:
        ...
        if self.auto_fix:
            ...
            issue.fixed = True
            issue.fix_description = (
                "Converted string-format authors to PEP 621 object format"
            )
        self.issues.append(issue)
    ...
    def _should_skip_string_author_check(self) -> bool:
        ...
        return bool(
            build_backend_match and "poetry" in build_backend_match.group(1).lower()
        )
