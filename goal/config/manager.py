    ...
    def load(self) -> Dict[str, Any]:
        ...
        if not self.exists():
            ...
            try:
                ...
                logger.warning(
                    "Failed to load config from %s: %s", self.config_path, exc
                )
                self._config = self._get_default_config()
    ...
    def _detect_project_name(self) -> str:
        ...
        if Path("pyproject.toml").exists():
            try:
                ...
                logger.debug(
                    "Unable to parse project name from pyproject.toml: %s", exc
                )
    ...
    def _detect_description(self) -> str:
        ...
        if Path("pyproject.toml").exists():
            try:
                ...
                logger.debug(
                    "Unable to parse project description from pyproject.toml: %s", exc
                )
        ...
        if Path("package.json").exists():
            try:
                ...
                logger.debug(
                    "Unable to parse project description from package.json: %s", exc
                )
