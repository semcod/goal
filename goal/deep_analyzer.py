...
class CodeChangeAnalyzer(CodeChangeAggregatorMixin):
    ...
    def analyze_file_diff(
        ...
        result["functional_areas"] = self._detect_functional_areas(
            result["added_entities"] + result["modified_entities"], new_content
        )
    ...
    def _analyze_python_diff(
        ...
        try:
            old_tree = (
                ast.parse(old_content) if old_content.strip() else ast.Module(body=[])
            )
            new_tree = (
                ast.parse(new_content) if new_content.strip() else ast.Module(body=[])
            )
        except SyntaxError:
            return result
        ...
        for name in old_names - new_names:
            result["removed_entities"].append(
                {"name": name, "type": old_entities[name]["type"]}
            )
                ...
                result["modified_entities"].append(
                    {
                        "name": name,
                        "type": new_e["type"],
                        "complexity_delta": new_e.get("complexity", 1)
                        - old_e.get("complexity", 1),
                    }
                )
        ...
        result["value_indicators"] = self._detect_value_indicators(
    ...
    def _detect_value_indicators(added_entities: List[Dict]) -> List[str]:
        ...
        for entity in added_entities:
            if any(
                d in str(entity.get("decorators", []))
                for d in ["click", "command", "option"]
            ):
