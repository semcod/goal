...
- [ ] AC-01: `goal governance check` recognizes the canonical source hub and
  returns success only after its deterministic Linux health contract passes.
- [ ] AC-02: Governed Goal delivery uses the same source-hub health runner
  instead of failing with `GOV-MANIFEST-001` or invoking the target-only
  `project/governance-check.sh` wrapper.
- [ ] AC-03: Missing/malformed hub inputs, unwired suites and failed suites
