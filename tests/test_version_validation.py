...
"""
    ...
    def test_update_badge_versions(self):
        ...
        with patch(
            "builtins.open", mock_open(read_data=self.test_readme_content)
        ) as mock_file:
            with patch("pathlib.Path.exists", return_value=True):
    ...
    def test_check_readme_badges(self):
        ...
        with patch("builtins.open", mock_open(read_data=self.test_readme_content)):
            with patch("pathlib.Path.exists", return_value=True):
                ...
                self.assertTrue(
                    all(badge["needs_update"] for badge in result["badges"])
                )
    ...
    def test_get_rubygems_version_success(self, mock_urlopen):
        ...
        mock_response = mock_urlopen.return_value.__enter__.return_value
        mock_response.read.return_value = json.dumps({"version": "3.2.1"}).encode()
