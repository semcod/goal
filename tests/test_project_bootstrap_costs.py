...
def test_calculate_ai_costs_uses_commit_diff_tuple_and_message_filter() -> None:
    ...
    commits = [
        (
            ai_commit,
            "diff --git a/goal.py b/goal.py\n@@ -1 +1 @@\n-print('a')\n+print('b')",
        ),
        (normal_commit, "diff --git a/README.md b/README.md"),
    ]

    with (
        patch("costs.git_parser.parse_commits", return_value=commits),
        patch("costs.calculator.ai_cost", return_value={"cost": 0.42}) as ai_cost_mock,
    ):
