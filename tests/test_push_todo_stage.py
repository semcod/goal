...
_TODO_STAGE_PATH = Path(__file__).resolve().parents[1] / "goal" / "push" / "stages" / "todo.py"
...
def test_todo_stage_stages_existing_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    (tmp_path / ".planfile" / "sprints" / "current.yaml").write_text("tickets: []\n", encoding="utf-8")
    ...
    with mock.patch.object(_MODULE.subprocess, "run", return_value=proc), mock.patch.object(
        _MODULE, "run_git", side_effect=_fake_run_git
    ):
